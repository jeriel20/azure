#requires -version 3
##########################################################################
# Script Name   : createDCGSaccounts-2008.ps1
# Version       : 4.0
...
if(!(test-path "C:\Scripts\createDCGSaccounts-2008.ps1"))
{
   Clear-Host
   Write-Host -foregroundcolor YELLOW "`n Script must be stored and executed from C:\Scripts directory!`n`n"
   "ERROR: createAccounts.ps1 not in c:\scripts directory" | Write-Log
   "===== STOP INSTALLATION =====" | Write-log
   EXIT
}
...
$prereqs = " This script will create all the required DCGS-A OUs, computers, security groups and accounts.`n`n"
$prereqs += " NOTES  : * The script only prompts for ONE PASSWORD which is used for ALL service accounts`n"
$prereqs += "        : * The script prompts for a Site Name used for a first child OU and inserted into`n"
$prereqs += "        : * The security group names and prefixed to all the account names. `n"
...
$prereqs += " Input  : * The script processes the .CSV and .TXT files located in the C:\Scripts directory`n"
$prereqs += "          * Edit these files using NOTEPAD to make any desired changes - be CAREFUL of format`n`n"
...
$securepwd = get-Password "Default Password for DCGS-A Accounts"
...
$siteName = get-Response "Site Name " "CYAN"
